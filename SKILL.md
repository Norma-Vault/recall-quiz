...
5. **Deliver compactly.** Use the host's native single-select control if available; otherwise a tight inline block — stem ≤ 25 words, options ≤ 12 words each — ending "(Reply with a letter, or say skip.)" Then stop and wait. Never reveal the answer in the same turn.
...
> Quick check: In a RAG pipeline, the **retrieval** step…
...
User picks C → "That's fine-tuning, which changes the weights. Retrieval (B) leaves the model untouched and pulls in relevant passages at query time."
